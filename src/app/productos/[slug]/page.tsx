      ...
      <section className="product-hero" id="inicio">
        ...
        <div className="container relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-[60px] lg:gap-[80px] items-center">
            <div>
              {/* Breadcrumb — enough top breathing room from navbar */}
              <div style={{ marginBottom: '24px' }} className="flex flex-wrap items-center gap-2 text-[13px] text-slate-500">
              ...
              {/* Badge — 16px below breadcrumb */}
              <div
                className="badge"
                style={{ marginBottom: '20px', background: product.iconBg, borderColor: product.iconBorder, color: product.themeColor }}
              >
                <span className="dot" style={{ background: product.themeColor }}></span>
                {product.category}
              </div>

              {/* Title — 16px below badge */}
            ...
            {/* Mockup — max-w-full + overflow-hidden prevents mobile overflow */}
            {product.mockupConfig && (
              <div className="animate-fade-up animate-delay-1 w-full max-w-full overflow-hidden">
                <div
                ...
                >
                  <div className="flex items-center gap-2 p-3 border-b" style={{ background: 'rgba(255,255,255,0.03)', borderColor: `${product.themeColor}1a` }}>
                    ...
                    <div className="flex-1 min-w-0 bg-white/5 border border-white/10 rounded px-2.5 py-1 text-[11px] text-slate-500 truncate">
                  ...
                  <div className="p-4 sm:p-5">
                    <div className="flex gap-3 mb-3.5">
                      <div className="flex-1 p-3 rounded-lg border" style={{ background: `${product.themeColor}1a`, borderColor: `${product.themeColor}33` }}>
                        <div className="text-[10px] text-slate-500 mb-1 font-semibold uppercase tracking-wider">{product.mockupConfig.metric1Label}</div>
                        <div className="text-xl font-extrabold text-white">{product.mockupConfig.metric1Value}</div>
                      </div>
                      <div className="flex-1 p-3 rounded-lg border" style={{ background: `rgba(34,197,94,0.08)`, borderColor: `rgba(34,197,94,0.15)` }}>
                        <div className="text-[10px] text-slate-500 mb-1 font-semibold uppercase tracking-wider">{product.mockupConfig.metric2Label}</div>
                        <div className="text-xl font-extrabold text-white">{product.mockupConfig.metric2Value}</div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-lg overflow-hidden">
                      ...
                      {product.mockupConfig.rows.map((row, i) => (
                        <div key={i} className="flex items-center gap-2.5 p-2.5 border-b border-white/5 last:border-none min-w-0">
                          <div className="w-7 h-7 rounded flex-shrink-0" style={{ background: row.avatarColor }}></div>
                          <div className="flex-1 min-w-0 bg-white/5 h-2 rounded-full"></div>
                          {row.statusText && (
                            <div className="flex-shrink-0 px-2 py-0.5 rounded text-[10px] font-bold" style={{ background: row.statusBg, color: row.statusColor }}>
                              {row.statusText}
                            </div>
                          )}
                        </div>
